...
class STDP_engine:
    def __init__(self, phi_p_e=0.01, phi_p_i=0.03, phi_n_e=0.015,
                 phi_n_i=0.045, w_max=1, tau_p=20, tau_n=20):
    ...
    def w_func(self, x, pre_w, is_exc, testing=True):
        ...
        # if testing:
        #     print("time delta", x, " | delta",
        #           a*exp(x/tau)*10000//1/10000, " | w", pre_w*10000//1/10000)
        return 5*a * exp(x / tau)
    ...
    def train(self, neuron):
        ...
        for i, (post_neuron, w) in enumerate(neuron.post_syn):
            for t_i in post_neuron.t_fired:
                ...
                w=(w if w>0 else 0)
                neuron.post_syn[i][1] = w+self.w_func(t_i-t_j, w, exc)
        ...
        for i, pre_neuron in neuron.pre_syn:
            ...
            for t_j in pre_neuron.t_fired:
                ...
                w=(w if w>0 else 0)
                pre_neuron.post_syn[i][1] = w+self.w_func(t_i-t_j, w, exc)
...
if __name__ == "__main__":
    ...
    from Ex_3.Learning_Alg import STDP_engine
    ...
    dt=0.03125;test_time=500;runtime=3000+test_time
    time_steps=int(runtime//dt)
    stdp_eng=STDP_engine(phi_p_e=0.01, phi_p_i=0.03,
        phi_n_e=0.015, phi_n_i=0.045, w_max=0.5, tau_p=20, tau_n=20)
    model=CustomModel(n_type=AELIF, excit_count=12, inhib_count=0, J=0.25, stdp_eng=stdp_eng,
    ...
    freq = lambda x,f: 1 if (x*dt-f*1)%100==0 else 0
    u_history=[]; i_history=[]; w_his=[]
    for t in range(time_steps):
        if t*dt%100==0: patter_select=1 if random()<0.5 else 0
        if patter_select:
            forced_spike_input=[freq(t,5),freq(t,10),freq(t,5),freq(t,5),freq(t,10),0,0,0,0,0]
            forced_spike_output=[freq(t,15), freq(t,1)] if model.stdp_eng!=None else []
        else:
            forced_spike_input=[0,0,0,0,0,freq(t,5),freq(t,10),freq(t,5),freq(t,5),freq(t,10)]
            forced_spike_output=[freq(t,1),freq(t,15)] if model.stdp_eng!=None else []
    ...
    min_t = 0 #time_steps+test_time
    # model.draw_graph()
    # stdp_eng.shape_of_w(-60, 60, 0.1, 1)
    plot_current([
    ...
    # plot_mv_ms(array(u_history), arange(0,runtime, dt), top=-40, bottom=-80)
    # plot_mv_ms(array(i_history), arange(0,runtime, dt))
    plot_raster(*generate_spike_data(model, runtime, dt), runtime, dt, min_t=min_t)
    w_his=np.array(w_his)
    plot_weight_spike(dt,runtime,model.pre_neurons[0], 0, w_his[:,0], max_x=min_t)
    plot_weight_spike(dt,runtime,model.pre_neurons[0], 1, w_his[:,1], max_x=min_t)
